...
dist = np.random.uniform(1,6,1)
# b) Random numbers from a normal distribution
dist = np.random.normal(1,5,1)
# c) Random integers
dist = np.random.randint(1,50,1)
# d) Random choice
import string
AllLetters = [i for i in string.ascii_lowercase] # This is a list of all
# Letters in the alphabet
list = np.unique(AllLetters)

# Without replacement
list = np.random.shuffle(AllLetters)
